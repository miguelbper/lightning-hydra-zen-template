...
class TestMNISTDataModule:
    ...
    def test_train_dataset(self, datamodule: MNISTDataModule):
        X, y = datamodule.train_dataset()
        N = len(datamodule.mnist_train)
        assert X.shape == (N, H * W)
        assert y.shape == (N,)

    def test_val_dataset(self, datamodule: MNISTDataModule):
        X, y = datamodule.val_dataset()
        N = len(datamodule.mnist_val)
        assert X.shape == (N, H * W)
        assert y.shape == (N,)

    def test_test_dataset(self, datamodule: MNISTDataModule):
